...
class UserLogoutAPIView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            token.blacklist()
            return Response({"status":1},status=status.HTTP_205_RESET_CONTENT)
...
class UserAPIView(RetrieveUpdateAPIView):
    ...
    """
    permission_classes = (IsAuthenticated,)
    serializer_class = serializers.CustomUserSerializerGet
...
class UserBookmarkAPIView(ListCreateAPIView):
    ...
    def post(self, request, pk):
        ...
        if user_profile:
            ...
            return Response({"status":1,"Msg":"Bookmark Successfull"},status=status.HTTP_200_OK)
        return Response({"status":0},status=status.HTTP_400_BAD_REQUEST)
