...
headers = {
	"X-RapidAPI-Key": "b76d195236msh797f424bdb5ec1ap1cd48ejsn794b89bbe753",
...
def yummly_autocomplete(request):
    ...
    if response.status_code == 200:
        ...
        return JsonResponse({"status":0,"error": "Failed to fetch data from Yummly2 API"}, status=500)
...
def yummly_search(request):
    ...
    if response.status_code == 200:
        ...
        return JsonResponse({"status":1,"data":parsed_recipes}, safe=False)
    else:
        return JsonResponse({"status":0,"error": "Failed to fetch data from Yummly2 API"}, status=response.status_code)
...
def category_feed(request):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return JsonResponse({"status":1,"error": "Failed to fetch data from Yummly2 API", "status_code": response.status_code}, status=response.status_code)
    except requests.exceptions.RequestException as e:
        return JsonResponse({"status":0,"error": str(e)}, status=500)
...
def yummly_feeds_list(start, limit, tag=''):
    ...
    if response.status_code == 200:
        ...
        return JsonResponse({"status":1,"data":filtered_data}, safe=False)
    else:
        return JsonResponse({"status":0,"error": "Failed to fetch data from Yummly2 API"}, status=response.status_code)
...
def get_list_similarities(request):
    ...
    if response.status_code == 200:
        ...
        return JsonResponse({"status":0,"error": "Failed to fetch data from the API"}, status=response.status_code)
...
def get_categories_list(request):
    ...
    if response.status_code == 200:
        ...
        return JsonResponse({"status":1,"data":simplified_categories})
    else:
        return JsonResponse({"status":0,"error": "Failed to fetch data from Yummly2 API"}, status=response.status_code)
